        ...
        while (is_connected.load(memory_order_acquire)) {
            try {
                ...
                if (!client_socket.poll(heartbeat_timespan,
                    ...
                    const vector<byte> packet =
                        cisco::common::serialize(heartbeat_req);
                    client_socket.sendBytes(packet.data(), packet.size());
                ...
                channel::EventChannel<channel::event::CTIEvent>::getInstance()
                    ->publish(channel::event::CTIEvent{received_packet});
                ...
                channel::EventChannel<
                    channel::event::CTIErrorEvent>::getInstance()
                    ->publish(channel::event::CTIErrorEvent{
                        getCTIServerHost(), channel::event::CTIErrorEvent::
                                                CTIErrorType::CONNECTION_LOST});
            }
        }
    }};
...
void CTIClient::handleEvent(const channel::event::Event *event) {
    ...
    case channel::event::EventType::ERROR_EVENT:
        ...
        case channel::event::ErrorType::CTI_ERROR:
            spdlog::debug(
                "CTI_Error_Event received, error_host: {}, cti_error_type: {}",
